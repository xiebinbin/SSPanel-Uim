...
CREATE TABLE IF NOT EXISTS `agent_bills` (
    ...
    `finance_user_id` bigint(20) unsigned NOT NULL COMMENT '财务 ID',
    ...
    `status` enum('NORMAL', 'CANCEL') DEFAULT 'NORMAL' COMMENT '资金流向 NORMAL成功 CANCEL撤销',
    `created_at` timestamp NULL COMMENT '创建时间',
    `updated_at` timestamp NULL COMMENT '更新时间',
    `canceled_at` timestamp NULL COMMENT '撤销时间',
    PRIMARY KEY (`id`),
    key user_id(`user_id`),
    key agent_user_id(`agent_user_id`),
    key finance_user_id(`finance_user_id`),
    key flow(`flow`)
