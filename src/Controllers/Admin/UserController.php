...
use App\Models\{AgentWallet, User, Shop, Bought, DetectBanLog};
use App\Services\{
    Auth,
    Mail,
    Config
...
class UserController extends AdminController
{
    ...
    public function update($request, $response, $args)
    {
        ...
        $isAgent = $request->getParam('is_agent');
        if ($isAgent == 1) {
            // 生成代理商钱包
            $agentWallet = AgentWallet::where('user_id', $user->id)->first();
        ...
        $user->is_finance = $request->getParam('is_finance');
        $user->is_operation = $request->getParam('is_operation');
        $user->is_agent = $isAgent;
        $user->agent_level = $request->getParam('agent_level');
